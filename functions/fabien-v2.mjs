...
// =============================================================================
// V2 — TOOL USE ANTHROPIC + AGENTIC LOOP
// =============================================================================
// Ce fichier est une version V2 de fabien.mjs qui ajoute l'accès en temps réel
// au catalogue Odoo via 3 outils (search_products, get_product_details,
// list_categories) appelés via /api/odoo (odoo-proxy.mjs).
//
// Route Netlify : /api/fabien-v2  (voir netlify.toml)
// Coexiste avec /api/fabien (V1 statique) pour permettre le rollback rapide.

const TOOLS = [
  {
    name: "search_products",
    description: "Recherche dans le catalogue Odoo FAIREKO filtre IA. Retourne 1 a 10 produits valides avec leurs caracteristiques techniques (lambda, feu, biosource, prix, niveau de confiance). A appeler AVANT toute recommandation produit, meme si tu penses connaitre le produit.",
    input_schema: {
      type: "object",
      properties: {
        query: {
          type: "string",
          description: "Mots-cles : nom produit ou famille (ex: 'pi-hemp', 'restaura', 'chanvre', 'chaux'). Optionnel."
        },
        category: {
          type: "string",
